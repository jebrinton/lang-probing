...
def check_tensor_devices(*tensors, names=None):
    """Check and print devices of tensors for debugging."""
    if names is None:
...
def collect_sentence_activations(model, dataloader, layers, tracer_kwargs=None):
    """
    Extrae activaciones mean-pooled por sentencia para capas específicas.
    
    Args:
        model: LanguageModel (nnsight)
        dataloader: DataLoader con batches (debe incluir 'tags', 'language')
        layers: Lista de números de capa a extraer
        tracer_kwargs: Argumentos para nnsight tracer
        
    Returns:
        list: Lista de dicts con "sentence_id", "language", "layer", "tags", "activation"
    """
    if tracer_kwargs is None:
        from .config import TRACER_KWARGS
        tracer_kwargs = TRACER_KWARGS
    ...
    with torch.no_grad():
        for batch in tqdm(dataloader, desc="Collecting activations"):
            ...
            # This dictionary will hold the PROXIES of the final pooled activations
            layer_pooled_acts = {} 

            with model.trace(batch_sentences, **tracer_kwargs):
                # We can access the tokenizer's output from within the trace.
                # This ensures we get the *correct* attention mask that matches the activations.
                # The 'input' attribute of the embedding module typically holds keyword arguments 
                # passed to the forward method, including 'attention_mask'.
                # We access the first positional argument which is the dictionary of tokenized inputs.
                ...
                for layer_num in layers:
                    ...
                    # Create a mask compatible with the activations' shape for broadcasting
                    # Use the attention mask generated by the model's tokenizer during the trace
                    mask = correct_attention_mask.unsqueeze(-1)  # Shape: (batch_size, seq_len, 1)

                    # Zero out the activations of padding tokens and sum over sequence
                    masked_acts = acts_proxy * mask
                    seq_sums = masked_acts.sum(dim=1)
                    
                    # Calculate mean over non-padding tokens
                    seq_lengths = correct_attention_mask.sum(dim=1, keepdim=True).float()  # Shape: (batch_size, 1)
                    
                    # Prevent division by zero for empty sequences
                    seq_lengths = torch.max(seq_lengths, torch.tensor(1.0).to(seq_lengths.device))
                    
                    pooled_acts_proxy = seq_sums / seq_lengths  # Shape: (batch_size, hidden_dim)

                    # 3. Save the FINAL result of the computation. 
                    # This tells nnsight "after all that, this is the tensor I want"
                    layer_pooled_acts[layer_num] = pooled_acts_proxy.save()

            # AFTER the context block, the .save()d proxies are populated with real tensors.
            # Now we access them using .value
            for layer_num in layers:
                # Get the computed tensor from the proxy
                pooled_acts = layer_pooled_acts[layer_num].value
                
                # Convert to numpy
                pooled_acts_np = pooled_acts.float().cpu().numpy()
                ...
                for i in range(len(batch_tags)):
                    results.append({
                        ...
                        "layer": layer_num,
                        "tags": batch_tags[i],
                        "activation": pooled_acts_np[i]
                    })
            
            sentence_id += len(batch_tags)
